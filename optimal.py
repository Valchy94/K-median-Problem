#Iz modula Pyclustering smo si sposodili nekaj algoritmov. Avtor le-teh je Andrei Novikov.
...
def manhattan_distance(a, b):
    ...
    return distance
#lahko bi vzela tudi svojo funkcijo razdalja ....

def median(points, indexes=None):
    """!
    @brief Calculate geometric median of input set of points using Manhattan distance.
    ...
    """
    # Mi bomo uporabili manhattansko razdaljo za izracun mediane...

    index_median = None
    distance = float('Inf')

    #range_points = None;   tega sploh ne rabimo!
    if (indexes is None):
        range_points = range(len(points)) #ce nimamo indexov za mediane, potem vzamemo kar vse
    else:
        range_points = indexes

    for index_candidate in range_points:
        distance_candidate = 0.0           #nastavimo razdaljo od kandidata do ostalih tock na 0
        for index in range_points:          #gledamo po vseh indeksih, ki pridejo v upostev za mediane
            distance_candidate += manhattan_distance(points[index_candidate], points[index])   #izracunamo, kaksna je njegova razdalja do ostalih tock

        if (distance_candidate < distance):
            distance = distance_candidate
            index_median = index_candidate

    return index_median  #dobimo indeks tocke, ki je najblizja vsem ostalim tockam v danem seznamu
...
class kmedoids:
    ...
    def process(self):
        ...
        while (changes > stop_condition):
              ...
              changes = max([manhattan_distance(self.__medoids[index], updated_medoids[index]) for index in
                             range(len(updated_medoids))])  # Fast solution
    ...
    def __update_clusters(self):
        ...
        for index_point in range(len(self.__pointer_data)):
            ...
            for index in range(len(self.__medoids)):
                ...
                dist = manhattan_distance(self.__pointer_data[index_point], self.__medoids[index])
            ...
            #clusters[index_optim] += [index_point]
